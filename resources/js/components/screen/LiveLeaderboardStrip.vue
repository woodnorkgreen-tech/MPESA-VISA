  ...
  <div class="w-full rounded-2xl border border-white/10 bg-black/25 px-4 py-3 lg:px-6">
    ...
    <TransitionGroup name="rank" tag="div" class="grid grid-cols-3 gap-2 lg:gap-4">
      <div v-for="entry in topEntries" :key="entry.id ?? entry.nickname"
        ...
        <span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-visa/30 text-[clamp(.75rem,1vw,1rem)] font-medium text-visa-gold">
          {{ entry.rank }}
        </span>
        ...
        <span class="shrink-0 text-[clamp(.75rem,1.4vw,1.5rem)] font-semibold tabular-nums text-visa-gold">{{ entry.trivia_score.toLocaleString() }}</span>
      </div>
    </TransitionGroup>
  </div>
</template>
